...
[[ $GITHUB_SHA == "$(gh api "repos/$GITHUB_REPOSITORY/git/ref/heads/main" --jq .object.sha)" ]] || exit 0
[[ $GITHUB_SHA == "$(gh api "repos/$GITHUB_REPOSITORY/compare/$GITHUB_SHA...$sha" --jq .merge_base_commit.sha)" ]] || exit 0
...
gh api "repos/$GITHUB_REPOSITORY/rules/branches/main" | jq -e 'any(.[]; .type == "pull_request" and .parameters.dismiss_stale_reviews_on_push == true) and any(.[]; .type == "required_status_checks" and .parameters.strict_required_status_checks_policy == true)' >/dev/null || exit 1
...
gh api "repos/$GITHUB_REPOSITORY/pulls/$number" | jq -e --arg sha "$sha" --arg repo "$GITHUB_REPOSITORY" 'select(.state == "open" and .draft == false and .head.sha == $sha and .base.ref == "main" and .base.repo.full_name == $repo and .user.id == 155234749 and .user.login == "daiksud")' >/dev/null || exit 0
[[ $GITHUB_SHA == "$(gh api "repos/$GITHUB_REPOSITORY/git/ref/heads/main" --jq .object.sha)" ]] || exit 0
